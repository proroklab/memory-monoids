...
class LinearAttention(eqx.Module):
    ...
    def __init__(self, input_dim, hidden_dim, key):
        ...
        self.mlp = nn.Sequential([
            nn.Linear(hidden_dim, hidden_dim, key=keys[4]), mish,
            nn.Linear(hidden_dim, hidden_dim, key=keys[5]), mish,
    ...
    def __call__(self, x, state, start, done):
        ...
        output = eqx.filter_vmap(self.ln)(eqx.filter_vmap(self.mlp)(output) + eqx.filter_vmap(self.skip)(x))
...
class LTQNetwork(eqx.Module):
    ...
    def __init__(self, obs_shape, act_shape, config, key):
        ...
        self.memory = [
            ...
            LinearAttention(config["recurrent_size"], config["recurrent_size"], keys[2])
        ]
    ...
    def initial_state(self, shape=tuple()):
        return [
            (
                jnp.zeros((*shape, 1, self.config['recurrent_size'], self.config['recurrent_size']), dtype=jnp.float32),
                jnp.zeros((*shape, 1, self.config['recurrent_size']), dtype=jnp.float32),
            )
            for _ in range(2)
        ]