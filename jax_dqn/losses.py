...
def segment_dqn_loss(q_network, q_target, segment, gamma):
    ...
    start = jnp.expand_dims(segment["start"], -1)
    done = jnp.expand_dims(segment["done"], -1)
    q_values, _ = eqx.filter_vmap(q_network, in_axes=(0, None, 0, 0))(segment["observation"], initial_state, start, done)
    ...
    next_q, _ = eqx.filter_vmap(q_target, in_axes=(0, None, 0, 0))(segment["next_observation"], initial_state, start, done)
...
def stream_dqn_loss(q_network, q_target, stream, gamma):
    ...
    initial_state = q_network.initial_state()
    q_values, _ = q_network(stream["observation"], stream["start"], stream["done"], initial_state)
    batch_index = jnp.arange(B)
    selected_q = q_values[
       batch_index, stream["action"]
    ]
    next_q, _ = q_target(stream["next_observation"], stream["start"], stream["done"], initial_state)
    target = jax.lax.stop_gradient(
